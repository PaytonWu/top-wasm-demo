...
#include "xstate_accessor_c/xstate_accessor_handle.h"
...
uint64_t withdraw(xstate_accessor_handle_t state_accessor_handle, char const * const property_name, uint64_t const amount, char const * const symbol, int * ec) {
    if (state_accessor_handle == nullptr || state_accessor_handle->unused_handle == nullptr) {
        if (ec != nullptr) {
    ...
    if (property_name == nullptr) {
        if (ec != nullptr) {
            *ec = static_cast<int>(state_accessor::error::xerrc_t::property_name_out_of_range);
        }
    ...
    assert(state_accessor_handle != nullptr && state_accessor_handle->unused_handle != nullptr);
    auto * state_accessor = reinterpret_cast<state_accessor::xstate_accessor_t *>(state_accessor_handle->unused_handle);
